...
pub fn eval(expr: &Expression, env: &mut Environment) -> Result<Expression, Error> {
    match expr {
        Expression::Symbol(k) => {
            env_get(k, env)
                .ok_or(Error(format!("unexpected symbol `{}`", k)))
        },
        ...
        Expression::List(list) => {
            let first_form = list
                .first()
                .ok_or(Error("expected non-empty list".to_string()))?;
...
fn env_for_lambda<'a>(
    ...
    if ks.len() != arg_forms.len() {
        return Err(Error(format!("expected {} arguments, got {}", ks.len(), arg_forms.len())));
    }
    ...
    Ok(Environment {data, outer: Some(outer_env)})
}
...
pub fn eval_lambda_args(arg_forms: &[Expression]) -> Result<Expression, Error> {
    let params_expr = arg_forms.first()
        .ok_or(Error("expected arguments".to_string()))?;
    let body_expr = arg_forms.get(1)
        .ok_or(Error("expected body".to_string()))?;
    if arg_forms.len() > 2 {
...
pub fn eval_defn_args(exprs: &[Expression], env: &mut Environment) -> Result<Expression, Error> {
    ...
    let params_expr = exprs.get(1)
        .ok_or(Error("expected argument list".to_string()))?;
    let body_expr = exprs.get(2)
        .ok_or(Error("expected function body".to_string()))?;
    
    env.data.insert(name, Expression::Lambda(Lambda {
        body: Rc::new(body_expr.clone()),
        params: Rc::new(params_expr.clone()),
    }));
